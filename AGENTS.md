...
- Monorepo-style packages: shared UI (`@investwise/ui`), addon SDK, and addon
  dev tools.
...
- Packages: `packages/`
  ...
  - `ui` — Shared UI lib (`@investwise/ui`)
...
- Use Tailwind v4 (CSS-first) and shared components from `@investwise/ui`.
...
- Build UI with components from `@investwise/ui` and Tailwind.
...
- Scaffold: `npx @investwise/addon-dev-tools create <my-addon>`
- Start addon dev server: `npm run dev:server` (from addon dir)
- Run InvestWise (desktop) with addon dev mode:
...
- UI/styling: use Tailwind utilities; reuse `@investwise/ui` components where
  possible.
...
| Full web mode | `pnpm run dev:web` |
| Run tests | `pnpm test` |
