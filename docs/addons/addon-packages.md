...
A comprehensive guide to the packages available for developing InvestWise
addons.
...
```bash
npm install @investwise/ui
```

**What it provides:**

- Pre-built UI components consistent with InvestWise's design
- Radix UI primitives
...
```bash
npm install -D @investwise/addon-dev-tools
```

**What it provides:**

- `investwise` CLI command
...
All Radix UI components are available through `@investwise/ui`:

```typescript
// Dialog components
import { Dialog, DialogContent, DialogTrigger } from "@investwise/ui";

// Form components
import { Input, Label, Checkbox, Select } from "@investwise/ui";

// Navigation
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@investwise/ui";

// Feedback
import { Alert, AlertDescription, Toast } from "@investwise/ui";
```
...
Available through `@investwise/ui`:
...
```typescript
import { cn } from '@investwise/ui'; // clsx + tailwind-merge
...
```json
{
  "name": "my-addon",
  "version": "1.0.0",
  "description": "My InvestWise addon",
  "type": "module",
  "main": "dist/addon.js",
  "keywords": ["investwise", "addon"],
  "license": "MIT",
  "scripts": {
    "build": "vite build",
...
| SDK Version | InvestWise Version | React Version |
| ----------- | ------------------- | ------------- |
| 1.0.0       | 1.0.0+              | ^19.1.1       |
...
npm init -y
npm install @investwise/addon-sdk @investwise/ui react react-dom
npm install -D @investwise/addon-dev-tools @vitejs/plugin-react vite typescript
```
...
{
  "dependencies": {
    "@investwise/addon-sdk": "workspace:*",
    "@investwise/ui": "workspace:*",
    "@investwise/addon-dev-tools": "workspace:*"
  }
}
```
...
### @investwise/addon-dev-tools Development Server
...
3. **Notifies InvestWise** to reload the addon
...
// InvestWise automatically discovers addons on these ports
```
...
```json
{
  "scripts": {
    ...
    "build": "vite build",
    "clean": "rm -rf dist",
    "bundle": "npm run build && investwise package",
    "lint": "tsc --noEmit"
  }
}
```
...
- No need to manually reload InvestWise
- Preserves addon state when possible

**Error Handling:**

- Build errors shown in terminal
- Runtime errors displayed in InvestWise dev console
...
2. **Open InvestWise** - addons are auto-discovered

3. **Make changes** to your addon code

4. **See changes instantly** in InvestWise
...
```bash
# Check InvestWise console for discovery logs
