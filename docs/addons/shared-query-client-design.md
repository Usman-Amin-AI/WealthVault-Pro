...
the main InvestWise application and addons, enabling automatic cache
...
(window as any).__investwise_query_client__ = queryClient;
```
...
```typescript
// Shared in @investwise/addon-sdk
...
```typescript
import { QueryKeys } from "@investwise/addon-sdk";
```
...
components of the InvestWise ecosystem.
