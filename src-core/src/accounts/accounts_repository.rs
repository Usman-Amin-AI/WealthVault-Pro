...
use crate::schema::accounts::dsl::{accounts as accounts_dsl, id, is_active, name};
...
impl AccountRepositoryTrait for AccountRepository {
    ...
    async fn update(&self, account_update: AccountUpdate, profile_id: &str) -> Result<Account> {
        ...
        self.writer
            .exec(move |conn| {
                ...
                account_db.profile_id = existing.profile_id;

                diesel::update(accounts_dsl.find(&account_db.id))
                    .set(&account_db)
                    .execute(conn)?;
    ...
    fn get_by_id(&self, account_id: &str, profile_id: &str) -> Result<Account> {
        let mut conn = get_connection(&self.pool)?;

        let account = accounts_dsl
            .select(AccountDB::as_select())
            .find(account_id)
    ...
    fn list(
        ...
        let mut query = accounts_dsl.filter(accounts::profile_id.eq(profile_id_param)).into_boxed();
    ...
    async fn delete(&self, account_id_param: &str, profile_id_param: &str) -> Result<usize> {
        let id_to_delete_owned = account_id_param.to_string();
        let profile_id_owned = profile_id_param.to_string();
        self.writer
            .exec(move |conn| {
                let affected_rows = diesel::delete(accounts_dsl.find(id_to_delete_owned).filter(accounts::profile_id.eq(profile_id_owned))).execute(conn)?;
                Ok(affected_rows)
            })
            .await
    }
