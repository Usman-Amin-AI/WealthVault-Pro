...
impl<E: DbTransactionExecutor + Send + Sync + Clone> AccountServiceTrait for AccountService<E> {
    ...
    async fn create_account(&self, new_account: NewAccount) -> Result<Account> {
        ...
        let mut new_account_for_tx = new_account.clone();
        if new_account_for_tx.profile_id.is_none() {
            new_account_for_tx.profile_id = Some("default_profile".to_string());
        }
    ...
    async fn update_account(&self, account_update: AccountUpdate, profile_id: &str) -> Result<Account> {
        (*self.repository).update(account_update, profile_id).await
    }

    /// Retrieves an account by its ID
    fn get_account(&self, account_id: &str, profile_id: &str) -> Result<Account> {
        (*self.repository).get_by_id(account_id, profile_id)
    }
    ...
    fn list_accounts(
        ...
        (*self.repository).list(is_active_filter, account_ids, profile_id)
    }

    /// Lists all accounts
    fn get_all_accounts(&self, profile_id: &str) -> Result<Vec<Account>> {
        (*self.repository).list(None, None, profile_id)
    }

    /// Lists only active accounts
    fn get_active_accounts(&self, profile_id: &str) -> Result<Vec<Account>> {
        self.list_accounts(Some(true), None, profile_id)
    }

    /// Retrieves multiple accounts by their IDs
    fn get_accounts_by_ids(&self, account_ids: &[String], profile_id: &str) -> Result<Vec<Account>> {
        self.list_accounts(None, Some(account_ids), profile_id)
    }

    /// Deletes an account by its ID
    async fn delete_account(&self, account_id: &str, profile_id: &str) -> Result<()> {
        (*self.repository).delete(account_id, profile_id).await?;
        Ok(())
    }
}
