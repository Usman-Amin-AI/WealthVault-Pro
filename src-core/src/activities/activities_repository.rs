...
impl ActivityRepositoryTrait for ActivityRepository {
    fn get_activity(&self, activity_id: &str, profile_id: &str) -> Result<Activity> {
        ...
        let activity_db = activities::table
            ...
            .filter(accounts::profile_id.eq(profile_id))
    ...
    fn search_activities(
        ...
        let account_id_filter_owned = account_id_filter.clone();
        let activity_type_filter_owned = activity_type_filter.clone();
        let asset_id_keyword_owned = asset_id_keyword.clone();
        ...
        let build_query = || {
            let mut query = activities::table
                ...
                .filter(accounts::is_active.eq(true))
                .filter(accounts::profile_id.eq(profile_id_owned.clone()))
                .into_boxed();

            if let Some(ref account_ids) = account_id_filter_owned {
                query = query.filter(activities::account_id.eq_any(account_ids));
            }
            if let Some(ref activity_types) = activity_type_filter_owned {
                query = query.filter(activities::activity_type.eq_any(activity_types));
            }
            if let Some(ref keyword) = asset_id_keyword_owned {
    ...
    async fn create_activity(&self, new_activity: NewActivity, _profile_id: &str) -> Result<Activity> {
    ...
    async fn update_activity(&self, activity_update: ActivityUpdate, _profile_id: &str) -> Result<Activity> {
    ...
    async fn delete_activity(&self, activity_id: String, _profile_id: &str) -> Result<Activity> {
        self.writer
            .exec(move |conn: &mut SqliteConnection| -> Result<Activity> {
    ...
    fn calculate_average_cost(&self, account_id: &str, asset_id: &str, profile_id: &str) -> Result<Decimal> {
        let mut conn = get_connection(&self.pool)?;
        ...
        let result: AverageCost = diesel::sql_query(
            ...
            WITH running_totals AS (
                ...
                WHERE activities.account_id = ?1 AND activities.asset_id = ?2
                  AND activities.activity_type IN ('BUY', 'TRANSFER_IN')
                  AND accounts.profile_id = ?3
            )
            SELECT
                CASE
    ...
    fn get_import_mapping(&self, some_account_id: &str, _profile_id: &str) -> Result<Option<ImportMapping>> {
        let mut conn = get_connection(&self.pool)?;
    ...
    async fn create_activities(&self, activities_vec: Vec<NewActivity>, _profile_id: &str) -> Result<usize> {
    ...
    fn get_income_activities_data(&self, profile_id: &str) -> Result<Vec<IncomeData>> {
        let mut conn = get_connection(&self.pool)?;
    ...
    fn get_first_activity_date_overall(&self, profile_id: &str) -> Result<DateTime<Utc>> {
        let mut conn = get_connection(&self.pool)?;
