...
pub struct SchwabIntegration {
    client_id: String,
    client_secret: String,
    http_client: Client,
}

impl SchwabIntegration {
...
fn build_activity_import(record: &Value, account_id: &str) -> ActivityImport {
    ...
    let account_name = parse_string(record.get("account_name"));

    ActivityImport {
        id: parse_string(record.get("transaction_id")).or_else(|| parse_string(record.get("id"))),
        date: parse_date(record.get("transaction_date")),
        symbol,
        activity_type: map_activity_type(record.get("transaction_type")),
        ...
        fee: parse_decimal(record.get("commission")).max(parse_decimal(record.get("fee"))),
...
#[async_trait]
impl BrokerIntegration for SchwabIntegration {
    fn get_authorization_url(&self, redirect_uri: &str) -> Result<String, AppError> {
        let auth_url = format!(
            "https://api.schwab.com/v1/oauth/authorize?response_type=code&client_id={}&redirect_uri={}&scope=transactions",
    ...
    async fn exchange_code_for_token(
        ...
        let token_endpoint = "https://api.schwab.com/v1/oauth/token";
        let response = self
            .http_client
            .post(token_endpoint)
            .form(&[
    ...
    async fn fetch_activities(
        ...
        let mut request = self
            .http_client
            .get(format!(
                "https://api.schwab.com/v1/accounts/{}/transactions",
                account_id
            ))
