    ...
    fn calculate_deposits_by_period(
        ...
        let deposit_activities =
            self.activity_repository
                .get_deposit_activities(account_ids, start_date, end_date, profile_id)?;
...
impl ContributionLimitServiceTrait for ContributionLimitService {
    fn get_contribution_limits(&self, profile_id: &str) -> Result<Vec<ContributionLimit>> {
        self.limit_repository.get_contribution_limits(profile_id)
    }
    ...
    async fn create_contribution_limit(
        ...
        self.limit_repository
            .create_contribution_limit(new_limit, profile_id)
            .await
    }
    ...
    async fn update_contribution_limit(
        ...
        self.limit_repository
            .update_contribution_limit(id, updated_limit, profile_id)
            .await
    }

    async fn delete_contribution_limit(&self, id: &str, profile_id: &str) -> Result<()> {
        self.limit_repository.delete_contribution_limit(id, profile_id).await
    }
    ...
    fn calculate_deposits_for_contribution_limit(
        ...
        let limit = self.limit_repository.get_contribution_limit(limit_id, profile_id)?;
        ...
        if let (Some(start_str), Some(end_str)) = (limit.start_date, limit.end_date) {
            ...
            let end = NaiveDateTime::parse_from_str(&end_str, "%Y-%m-%dT%H:%M:%S%.3fZ")
                .map_err(|e| Error::Validation(ValidationError::DateTimeParse(e)))?;
            self.calculate_deposits_by_period(&account_ids, start, end, base_currency, profile_id)
        } else {
