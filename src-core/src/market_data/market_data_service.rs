...
pub struct MarketDataService {
    ...
    live_engine: crate::market_data::live_market_engine::LiveMarketEngine,
}
...
impl MarketDataServiceTrait for MarketDataService {
    ...
    async fn get_live_prices(&self, symbols: &[String]) -> Result<HashMap<String, crate::market_data::market_data_model::LivePrice>> {
        let mut result = HashMap::new();
        let mut missing_symbols = Vec::new();

        for sym in symbols {
            if let Some(price) = self.live_engine.prices.get(sym) {
                result.insert(sym.clone(), price.clone());
            } else {
        ...
        if !missing_symbols.is_empty() {
            let db_prices = self.repository.get_live_prices(&missing_symbols)?;
            for (sym, price) in db_prices {
                result.insert(sym.clone(), price.clone());
                self.live_engine.prices.insert(sym, price);
            }
        }
        Ok(result)
    }

    async fn start_live_stream(&self, symbols: Vec<String>, provider: &str) -> Result<()> {
        let provider_key = provider.trim().to_lowercase();
        match provider_key.as_str() {
            "yahoo" | "yahoo_finance" | "yahoo-finance" => {
                self.live_engine.start_yahoo(symbols);
            },
            "alpaca" => {
                let api_key = self.resolve_live_provider_secret("ALPACA", &["alpaca_api_key"])?;
                let api_secret = self.resolve_live_provider_secret("ALPACA", &["alpaca_api_secret"])?;
                if api_key.is_empty() || api_secret.is_empty() {
                    return Err(crate::errors::Error::Secret(
                        "Alpaca live streaming requires both API key and secret".to_string(),
                    ));
                }
                self.live_engine.start_alpaca(symbols, api_key, api_secret);
            },
            "polygon" | "polygon.io" | "polygon_io" => {
                let api_key = self.resolve_live_provider_secret("POLYGON", &["polygon_api_key"])?;
                if api_key.is_empty() {
                    return Err(crate::errors::Error::Secret(
                        "Polygon live streaming requires an API key".to_string(),
                    ));
                }
                self.live_engine.start_polygon(symbols, api_key);
            },
            other => {
    ...
    fn subscribe_live_prices(&self) -> tokio::sync::broadcast::Receiver<crate::market_data::market_data_model::LivePrice> {
        self.live_engine.broadcast_tx.subscribe()
    }
}

impl MarketDataService {
    fn resolve_live_provider_secret(&self, provider_id: &str, legacy_names: &[&str]) -> Result<String> {
        let mut last_error: Option<String> = None;

        for candidate in std::iter::once(provider_id).chain(legacy_names.iter().copied()) {
            match self.secret_store.get_secret(candidate) {
                Ok(Some(secret)) if !secret.trim().is_empty() => return Ok(secret),
                Ok(Some(_)) => {
                    debug!("Live provider secret '{}' was present but empty", candidate);
                }
                Ok(None) => {
                    debug!("Live provider secret '{}' was not configured", candidate);
                }
                Err(err) => {
        ...
        if let Some(err) = last_error {
            warn!("Unable to resolve live provider credentials: {}", err);
        }
    ...
    pub async fn new(
        ...
        let live_engine = crate::market_data::live_market_engine::LiveMarketEngine::new(repository.clone());
