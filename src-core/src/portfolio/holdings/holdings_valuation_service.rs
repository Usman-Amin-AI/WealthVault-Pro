...
use crate::market_data::market_data_model::{LatestQuotePair, Quote};
...
impl HoldingsValuationService {
    ...
    async fn fetch_batch_quote_data(
        ...
        let mut latest_quote_pairs = if !required_symbols.is_empty() {
        ...
        if !required_symbols.is_empty() {
            let live_prices = self
                .market_data_service
                .get_live_prices(&required_symbols)
                .await?;

            for (symbol, live_price) in live_prices {
                let quote = Quote::from(&live_price);
                if let Some(existing_pair) = latest_quote_pairs.get_mut(&symbol) {
                    existing_pair.latest = quote;
                } else {
                    latest_quote_pairs.insert(
                        symbol.clone(),
                        LatestQuotePair {
                            latest: quote,
                            previous: None,
