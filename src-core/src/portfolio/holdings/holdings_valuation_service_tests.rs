...
mod tests {
    ...
    impl MarketDataServiceTrait for MockMarketDataService {
        ...
        async fn get_live_prices(&self, _symbols: &[String]) -> Result<HashMap<String, LivePrice>> {
            Ok(HashMap::new())
        }

        async fn start_live_stream(&self, _symbols: Vec<String>, _provider: &str) -> Result<()> {
            Ok(())
        }

        fn subscribe_live_prices(&self) -> tokio::sync::broadcast::Receiver<LivePrice> {
