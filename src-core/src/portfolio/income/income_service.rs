...
pub trait IncomeServiceTrait: Send + Sync {
    fn get_income_summary(&self, profile_id: &str) -> Result<Vec<IncomeSummary>>;
}
...
impl IncomeServiceTrait for IncomeService {
    fn get_income_summary(&self, profile_id: &str) -> Result<Vec<IncomeSummary>> {
        ...
        let activities = match self.activity_repository.get_income_activities_data(profile_id) {
        ...
        let oldest_date = match self.activity_repository.get_first_activity_date_overall(profile_id) {
            Ok(date) => date,
            Err(e) => {
