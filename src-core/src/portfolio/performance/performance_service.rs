use crate::constants::{DECIMAL_PRECISION, PORTFOLIO_TOTAL_ACCOUNT_ID, get_total_account_id};
...
impl PerformanceService {
    ...
    async fn calculate_account_performance_summary(
        ...
        let (start_point, end_point, actual_start_date, actual_end_date, currency): (
            ...
            NaiveDate,
            String,
        ) = self.get_account_boundary_data(account_id, start_date, end_date, profile_id)?;
...
impl PerformanceServiceTrait for PerformanceService {
    ...
    async fn calculate_performance_history(
        ...
        match item_type {
            "account" => {
                let resolved_item_id = if item_id == PORTFOLIO_TOTAL_ACCOUNT_ID {
                    get_total_account_id(profile_id)
                } else {
                    item_id.to_string()
                };
                self.calculate_account_performance(&resolved_item_id, start_date, end_date, profile_id)
                    .await
            }
            "symbol" => {
    ...
    async fn calculate_performance_summary(
        ...
        match item_type {
            "account" => {
                ...
                self.calculate_account_performance_summary(&resolved_item_id, start_date, end_date, profile_id)
                    .await
            }
            "symbol" => {
    ...
    fn calculate_accounts_simple_performance(
        ...
        let total_account_id = get_total_account_id(profile_id);
        if !account_ids.contains(&total_account_id) {
            ids_to_fetch.push(total_account_id.clone());
        }

        // 1. Fetch the *absolute* latest record for each account
        let latest_daily_valuations = self
            .valuation_service
            .get_latest_valuations(&ids_to_fetch, profile_id)?;
        ...
        for (prev_date, ids) in prev_dates_needed {
            match self
                .valuation_service
                .get_valuations_on_date(&ids, prev_date, profile_id)
            {
                Ok(records) => {
