...
pub trait SnapshotServiceTrait: Send + Sync {
    ...
    async fn calculate_holdings_snapshots(&self, account_ids: Option<&[String]>, profile_id: &str) -> Result<usize>;
    ...
    async fn calculate_total_portfolio_snapshots(&self, profile_id: &str) -> Result<usize>;
}
...
impl SnapshotService {
    ...
    fn create_total_virtual_account(&self, profile_id: &str) -> Account {
        let now = Utc::now().naive_utc();
        Account {
            id: get_total_account_id(profile_id),
    ...
    async fn calculate_holdings_snapshots_internal(
        ...
        let (accounts_to_process, all_activities, min_activity_date, calculation_end_date) =
            self.fetch_required_data(account_ids_param, profile_id)?;
    ...
    fn fetch_required_data(
        ...
        let calculate_total = account_ids_param
            .map(|ids| ids.iter().any(|id| *id == get_total_account_id(profile_id)))
            .unwrap_or(true); // ← implicitly includes TOTAL when param == None
        ...
        match account_ids_param {
            Some(ids) => {
                for id in ids {
                    if id == &get_total_account_id(profile_id) {
                        continue;
                    } // TOTAL is virtual
                    if let Ok(acc) = self.account_repository.get_by_id(id, profile_id) {
                        if acc.is_active {
            ...
            None => {
                for acc in self.account_repository.list(Some(true), None, profile_id)? {
                    account_ids_to_fetch_activities.push(acc.id.clone());
                    accounts_to_process.insert(acc.id.clone(), acc);
                }
            }
        ...
        if calculate_total {
            accounts_to_process.insert(
                ...
                self.create_total_virtual_account(profile_id),
            );
        }
        ...
        let all_activities = if !account_ids_to_fetch_activities.is_empty() {
            self.activity_repository
                .get_activities_by_account_ids(&account_ids_to_fetch_activities, profile_id)?
        } else {
            Vec::new()
        };
    ...
    fn preprocess_data(
        ...
        if accounts_to_process.contains_key(&get_total_account_id(profile_id)) {
            ...
            if !total_activities_by_date.is_empty() {
                ...
                if !account_ids_with_activity.is_empty() {
                    account_ids_with_activity.insert(get_total_account_id(profile_id));
                }
            }
        }
            ...
            if effective_start_date <= calculation_end_date {
                if let Some(snapshot) = initial_snapshot_for_acc {
                    ...
                    let day_before_effective_start = effective_start_date
                        .pred_opt().map(|d| d).unwrap_or(effective_start_date);
                    start_keyframes.insert(
                        acc_id.clone(),
    ...
    fn generate_total_portfolio_snapshot_for_date(
        ...
        for (individual_acc_id, individual_snapshot) in individual_snapshots_on_date {
            ...
            if individual_acc_id == &get_total_account_id(profile_id) {
                continue;
            }
            ...
            for pos in individual_snapshot.positions.values() {
                let agg_pos = aggregated_positions
                    ...
                    .or_insert_with(|| Position {
                        id: format!("{}_{}", pos.asset_id, get_total_account_id(profile_id)),
                        account_id: get_total_account_id(profile_id),
    ...
    async fn calculate_total_portfolio_snapshots_impl(&self, profile_id: &str) -> Result<usize> {
        debug!("Starting calculation of TOTAL portfolio snapshots (based on stored individual keyframes).");

        let mut all_individual_accounts = self.account_repository.list(Some(true), None, profile_id)?;
        if all_individual_accounts.is_empty() {
            warn!("No active accounts found. Cannot generate TOTAL snapshots.");
            self.snapshot_repository
                .overwrite_all_snapshots_for_account(&get_total_account_id(profile_id), &[])
                .await?;
            return Ok(0);
        }
        ...
        let all_individual_keyframes = self
            .snapshot_repository
            .get_all_active_account_snapshots(None, None, profile_id)?;
        ...
        for keyframe in all_individual_keyframes {
            if keyframe.account_id == get_total_account_id(profile_id) {
                continue;
            }
...
impl SnapshotServiceTrait for SnapshotService {
    async fn calculate_holdings_snapshots(&self, account_ids: Option<&[String]>, profile_id: &str) -> Result<usize> {
        self.calculate_holdings_snapshots_internal(account_ids, false, profile_id)
            .await
    }
    ...
    async fn force_recalculate_holdings_snapshots(
        ...
        self.calculate_holdings_snapshots_internal(account_ids, true, profile_id)
            .await
    }
    ...
    fn get_daily_holdings_snapshots(
        ...
        let mut current_state = match initial_state_result? {
            ...
            None => {
                ...
                let account_details =
                    self.account_repository.get_by_id(account_id, profile_id).or_else(|_| {
                        if account_id == get_total_account_id(profile_id) {
                            Ok(self.create_total_virtual_account(profile_id))
                        } else {
    ...
    async fn calculate_total_portfolio_snapshots(&self, profile_id: &str) -> Result<usize> {
        self.calculate_total_portfolio_snapshots_impl(profile_id).await
    }
}
