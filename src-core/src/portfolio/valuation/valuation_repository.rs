...
pub trait ValuationRepositoryTrait: Send + Sync {
    ...
    fn load_latest_valuation_date(&self, account_id: &str, profile_id: &str) -> Result<Option<NaiveDate>>;
    async fn delete_valuations_for_account(&self, account_id: &str, profile_id: &str) -> Result<()>;
...
impl ValuationRepositoryTrait for ValuationRepository {
    ...
    fn get_historical_valuations(
        ...
        let resolved_account_id = if input_account_id == crate::constants::PORTFOLIO_TOTAL_ACCOUNT_ID {
            crate::constants::get_total_account_id(profile_id)
        } else {
            input_account_id.to_string()
        };

        let mut query = daily_account_valuation::table
            .filter(account_id.eq(resolved_account_id))
    ...
    fn load_latest_valuation_date(&self, input_account_id: &str, profile_id: &str) -> Result<Option<NaiveDate>> {
    ...
    async fn delete_valuations_for_account(&self, input_account_id: &str, profile_id: &str) -> Result<()> {
        let account_id_owned = if input_account_id == crate::constants::PORTFOLIO_TOTAL_ACCOUNT_ID {
            crate::constants::get_total_account_id(profile_id)
        } else {
            input_account_id.to_string()
        };
    ...
    fn get_latest_valuations(
        ...
        let mut conn = get_connection(&self.pool)?;
        let resolved_account_ids: Vec<String> = input_account_ids
            .iter()
            .map(|account_id_input| {
                if account_id_input == crate::constants::PORTFOLIO_TOTAL_ACCOUNT_ID {
                    crate::constants::get_total_account_id(profile_id)
                } else {
                    account_id_input.to_string()
                }
            })
            .collect();

        let placeholders: String = resolved_account_ids
            .iter()
            .map(|_| "?")
            .collect::<Vec<&str>>()
            .join(", ");
        ...
        let sql = format!(
            ...
            INNER JOIN (
                ...
                WHERE account_id IN ({})
                GROUP BY account_id
            ) AS latest ON v.account_id = latest.account_id AND v.valuation_date = latest.max_date",
            placeholders
        );

        let mut query_builder = sql_query(sql).into_boxed::<Sqlite>();

        for acc_id_str in resolved_account_ids {
