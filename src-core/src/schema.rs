...
diesel::table! {
    app_settings (profile_id, setting_key) {
        profile_id -> Text,
        setting_key -> Text,
        setting_value -> Text,
    }
...
diesel::joinable!(broker_connections -> accounts (account_id));
diesel::joinable!(contribution_limits -> profiles (profile_id));
diesel::joinable!(goals -> profiles (profile_id));
diesel::joinable!(goals_allocation -> accounts (account_id));
diesel::joinable!(goals_allocation -> goals (goal_id));
diesel::joinable!(profile_shares -> profiles (owner_profile_id));
