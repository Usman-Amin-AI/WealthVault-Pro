...
pub trait SettingsRepositoryTrait: Send + Sync {
    ...
    fn get_setting(&self, setting_key_param: &str, profile_id: &str) -> Result<String>;
...
impl SettingsRepositoryTrait for SettingsRepository {
    fn get_settings(&self, profile_id_param: &str) -> Result<Settings> {
        let mut conn = get_connection(&self.pool)?;
        let all_settings: Vec<(String, String)> = app_settings
            .filter(profile_id.eq(profile_id_param))
    ...
    async fn update_settings(&self, new_settings: &SettingsUpdate, profile_id_param: &str) -> Result<()> {
        let settings = new_settings.clone();
        let pid = profile_id_param.to_string();
        self.writer
            .exec(move |conn| {
    ...
    fn get_setting(&self, setting_key_param: &str, profile_id_param: &str) -> Result<String> {
        let mut conn = get_connection(&self.pool)?;
