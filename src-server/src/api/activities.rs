...
use investwise_core::activities::{
...
#[serde(untagged)]
enum SortWrapper {
    One(investwise_core::activities::Sort),
    Many(Vec<investwise_core::activities::Sort>),
}
...
async fn search_activities(
    ...
    let sort_normalized: Option<investwise_core::activities::Sort> = match body.sort {
...
async fn create_activity(
    ...
    let created = state.activity_service.create_activity(activity, "default_profile").await?;
...
async fn update_activity(
    ...
    let previous = state.activity_service.get_activity(&activity.id, "default_profile")?;
    let updated = state.activity_service.update_activity(activity, "default_profile").await?;
...
async fn save_activities(
    ...
    let result = state
        .activity_service
        .bulk_mutate_activities(request, "default_profile")
        .await?;
...
async fn delete_activity(
    ...
    let deleted = state.activity_service.delete_activity(id, "default_profile").await?;
...
async fn check_activities_import(
    ...
    let res = state
        .activity_service
        .check_activities_import(body.account_id, body.activities, "default_profile")
        .await?;
    Ok(Json(res))
}
...
async fn import_activities(
    ...
    let res = state
        .activity_service
        .import_activities(body.account_id, body.activities, "default_profile")
        .await?;
    trigger_activity_portfolio_job(
        state,
        res.iter()
...
async fn get_account_import_mapping(
    ...
    let res = state.activity_service.get_import_mapping(q.account_id, "default_profile")?;
    Ok(Json(res))
}
