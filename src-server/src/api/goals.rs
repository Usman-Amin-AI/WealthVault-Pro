...
use investwise_core::goals::goals_model::{Goal, GoalsAllocation, NewGoal};

async fn get_goals(State(state): State<Arc<AppState>>) -> ApiResult<Json<Vec<Goal>>> {
    let goals = state.goal_service.get_goals("default_profile")?;
    Ok(Json(goals))
}
...
async fn create_goal(
    ...
    let g = state.goal_service.create_goal(goal, "default_profile").await?;
...
async fn update_goal(
    ...
    let g = state.goal_service.update_goal(goal, "default_profile").await?;
    trigger_lightweight_portfolio_update(state.clone());
    Ok(Json(g))
}
...
async fn delete_goal(
    ...
    let _ = state.goal_service.delete_goal(id, "default_profile").await?;
...
async fn load_goals_allocations(
    ...
    let allocs = state.goal_service.load_goals_allocations("default_profile")?;
    Ok(Json(allocs))
}
...
async fn update_goal_allocations(
    ...
    let _ = state.goal_service.upsert_goal_allocations(allocs, "default_profile").await?;
    Ok(StatusCode::NO_CONTENT)
}
