...
use investwise_core::limits::{ContributionLimit, DepositsCalculation, NewContributionLimit};

async fn get_contribution_limits(
    State(state): State<Arc<AppState>>,
) -> ApiResult<Json<Vec<ContributionLimit>>> {
    let limits = state.limits_service.get_contribution_limits("default_profile")?;
    Ok(Json(limits))
}
...
async fn create_contribution_limit(
    ...
    let created = state
        .limits_service
        .create_contribution_limit(new_limit, "default_profile")
        .await?;
...
async fn update_contribution_limit(
    ...
    let updated = state
        .limits_service
        .update_contribution_limit(&id, updated, "default_profile")
        .await?;
    trigger_lightweight_portfolio_update(state.clone());
    Ok(Json(updated))
}
...
async fn delete_contribution_limit(
    ...
    state.limits_service.delete_contribution_limit(&id, "default_profile").await?;
...
async fn calculate_deposits_for_contribution_limit(
    ...
    let calc = state
        .limits_service
        .calculate_deposits_for_contribution_limit(&id, &base, "default_profile")?;
    Ok(Json(calc))
}
