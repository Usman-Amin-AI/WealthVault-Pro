...
async fn get_settings(
    ...
    let s = state.settings_service.get_settings(profile_id)?;
    Ok(Json(s))
}
...
async fn update_settings(
    ...
    state.settings_service.update_settings(&payload, profile_id).await?;
    let updated_settings = state.settings_service.get_settings(profile_id)?;
...
async fn is_auto_update_check_enabled(
    ...
    let enabled = state
        .settings_service
        .is_auto_update_check_enabled(profile_id)
        .unwrap_or(true);
    Ok(Json(enabled))
}
