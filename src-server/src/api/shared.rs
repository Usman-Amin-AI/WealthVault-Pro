...
pub const DEFAULT_PROFILE_ID: &str = "default_profile";

/// Preserve backward compatibility by normalizing optional profile IDs.
pub fn normalize_profile_id(profile_id: Option<&str>) -> &str {
    profile_id.unwrap_or(DEFAULT_PROFILE_ID)
}
...
impl PortfolioRequestBody {
    pub fn into_config(self, force_full_recalculation: bool, profile_id: String) -> PortfolioJobConfig {
        PortfolioJobConfig {
...
pub fn trigger_account_portfolio_job(state: Arc<AppState>, impact: AccountPortfolioImpact) {
    ...
    enqueue_portfolio_job(
        ...
        PortfolioJobConfig {
            ...
            profile_id: "default_profile".to_string(), // Keep default for triggered jobs unless refactored deeper
        },
    );
}
...
pub async fn process_portfolio_job(
    ...
    let active_accounts = state
        .account_service
        .list_accounts(Some(true), config.account_ids.as_deref(), &config.profile_id)
        .map_err(|err| {
    ...
    if !account_ids.is_empty() {
        ...
        let snapshot_result = if config.force_full_recalculation {
            state
                .snapshot_service
                .force_recalculate_holdings_snapshots(Some(ids_slice), &config.profile_id)
                .await
        } else {
            state
                .snapshot_service
                .calculate_holdings_snapshots(Some(ids_slice), &config.profile_id)
                .await
        };
    ...
    if let Err(err) = state
        .snapshot_service
        .calculate_total_portfolio_snapshots(&config.profile_id)
        .await
    {
    ...
    for account_id in account_ids {
        if let Err(err) = state
            .valuation_service
            .calculate_valuation_history(&account_id, config.force_full_recalculation, &config.profile_id)
            .await
        {
            let err_msg = format!(
...
pub fn trigger_activity_portfolio_job(state: Arc<AppState>, impacts: Vec<ActivityImpact>) {
    ...
    for impact in impacts {
        ...
        if let Some(currency) = impact.currency.as_deref() {
            match state.account_service.get_account(&impact.account_id, "default_profile") {
                Ok(account) => {
