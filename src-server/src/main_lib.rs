...
use investwise_core::{
    ...
    profiles::{
        profiles_repository::ProfileRepository, profiles_service::ProfileService,
    },
    secrets::SecretStore,
...
pub struct AppState {
    ...
    pub broker_service: Arc<investwise_core::brokers::service::BrokerService>,
    ...
    pub performance_service:
        Arc<dyn investwise_core::portfolio::performance::PerformanceServiceTrait + Send + Sync>,
...
pub async fn build_state(config: &Config) -> anyhow::Result<Arc<AppState>> {
    ...
    let settings = settings_service.get_settings("default_profile")?;
    let base_currency = Arc::new(RwLock::new(settings.base_currency));

    let profile_repo = ProfileRepository::new(pool.clone(), writer.clone());
    ...
    let performance_service = Arc::new(
        investwise_core::portfolio::performance::PerformanceService::new(
    ...
    let broker_service = Arc::new(investwise_core::brokers::service::BrokerService::new(
        settings_repo.clone(),
        pool.clone(),
    ));
