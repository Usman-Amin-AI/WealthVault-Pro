...
pub async fn get_activities(
    ...
    Ok(state.activity_service().get_activities("default_profile")?)
}
...
pub async fn create_activity(
    ...
    let result = state.activity_service().create_activity(activity, "default_profile").await?;
...
pub async fn update_activity(
    ...
    let original_activity = state.activity_service()
        .get_activity(&activity.id, "default_profile")
        .map_err(|e| format!("Failed to fetch existing activity: {}", e))?;

    let result = state.activity_service().update_activity(activity, "default_profile").await?;
...
pub async fn delete_activity(
    ...
    let result = state.activity_service()
        .delete_activity(activity_id, "default_profile")
        .await
...
pub async fn get_account_import_mapping(
    ...
    Ok(state.activity_service().get_import_mapping(account_id, "default_profile")?)
}
...
pub async fn check_activities_import(
    ...
    let result = state.activity_service()
        .check_activities_import(account_id, activities, "default_profile")
        .await?;
    Ok(result)
}
...
pub async fn import_activities(
    ...
    let result = state.activity_service()
        .import_activities(account_id.clone(), activities, "default_profile")
        .await?;
    emit_resource_changed(
        &handle,
        ResourceEventPayload::new(
