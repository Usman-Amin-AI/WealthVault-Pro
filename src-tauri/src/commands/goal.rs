...
pub async fn get_goals(state: State<'_, Arc<ServiceContext>>) -> Result<Vec<Goal>, String> {
    ...
    state.goal_service().get_goals("default_profile").map_err(|e| e.to_string())
}
...
pub async fn create_goal(
    ...
    let new_goal = state
        .goal_service()
        .create_goal(goal, "default_profile")
        .await
...
pub async fn update_goal(
    ...
    let updated_goal = state
        .goal_service()
        .update_goal(goal, "default_profile")
        .await
        .map_err(|e| e.to_string())?;
...
pub async fn delete_goal(
    ...
    let result = state
        .goal_service()
        .delete_goal(goal_id.clone(), "default_profile")
        .await
        .map_err(|e| e.to_string())?;
...
pub async fn update_goal_allocations(
    ...
    state
        .goal_service()
        .upsert_goal_allocations(allocations, "default_profile")
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn load_goals_allocations(
    ...
    state
        .goal_service()
        .load_goals_allocations("default_profile")
        .map_err(|e| e.to_string())
}
