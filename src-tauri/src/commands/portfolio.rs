...
pub async fn get_holdings(
    ...
    state
        .holdings_service()
        .get_holdings(&account_id, &base_currency, "default_profile")
        .await
...
pub async fn get_holding(
    ...
    state
        .holdings_service()
        .get_holding(&account_id, &asset_id, &base_currency, "default_profile")
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn get_historical_valuations(
    ...
    state
        .valuation_service()
        .get_historical_valuations(&account_id, from_date_opt, to_date_opt, "default_profile")
        .map_err(|e| e.to_string())
}
...
pub async fn get_latest_valuations(
    ...
    let ids_to_process = if account_ids.is_empty() {
        ...
        state
            .account_service()
            .get_active_accounts("default_profile")
    ...
    state
        .valuation_service()
        .get_latest_valuations(&ids_to_process, "default_profile")
        .map_err(|e| e.to_string())
}
...
pub async fn get_income_summary(
    ...
    state
        .income_service()
        .get_income_summary("default_profile")
        .map_err(|e| e.to_string())
}
...
pub async fn calculate_accounts_simple_performance(
    ...
    state
        .performance_service()
        .calculate_accounts_simple_performance(&ids_to_process, "default_profile") // Pass the potentially modified list
        .map_err(|e| e.to_string())
}
...
pub async fn calculate_performance_history(
    ...
    state
        .performance_service()
        .calculate_performance_history(&item_type, &item_id, start_date_opt, end_date_opt, "default_profile")
        .await
...
pub async fn calculate_performance_summary(
    ...
    state
        .performance_service()
        .calculate_performance_summary(&item_type, &item_id, start_date_opt, end_date_opt, "default_profile")
        .await
        .map_err(|e| format!("Failed to calculate performance: {}", e))
}
