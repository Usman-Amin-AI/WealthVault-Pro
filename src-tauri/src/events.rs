...
/// Event emitted when a live price update is received.
pub const LIVE_PRICE_UPDATE: &str = "market:live-price-update";
...
pub fn emit_live_price_update(handle: &tauri::AppHandle, payload: investwise_core::market_data::market_data_model::LivePrice) {
    handle.emit(LIVE_PRICE_UPDATE, &payload).unwrap_or_else(|e| {
        log::error!("Failed to emit {} event: {}", LIVE_PRICE_UPDATE, e);
    });
}
