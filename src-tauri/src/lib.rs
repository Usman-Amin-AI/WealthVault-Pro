...
mod desktop {
    ...
    pub fn setup(handle: AppHandle, app_data_dir: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        // Add tray icon and background-mode lifecycle so the desktop app can stay resident
        tray::setup(&handle);

        let mut rx = context.market_data_service().subscribe_live_prices();
        let handle_clone = handle.clone();
        tauri::async_runtime::spawn(async move {
            while let Ok(price) = rx.recv().await {
                crate::events::emit_live_price_update(&handle_clone, price);
            }
        });
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::market_data::start_live_stream,
            commands::market_data::get_live_prices,
            // Platform commands
            ...
            // Broker commands
            commands::brokers::get_auth_url,
            commands::brokers::handle_oauth_callback,
            commands::brokers::get_broker_connection,
            commands::brokers::fetch_broker_activities,
        ])
