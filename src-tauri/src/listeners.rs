...
use investwise_core::constants::PORTFOLIO_TOTAL_ACCOUNT_ID;
...
fn handle_account_resource_change(handle: AppHandle, event: &ResourceEventPayload) {
    ...
    if let (Some(currency), Some(ctx)) = (&account_currency, context.as_ref()) {
        match ctx.settings_service().get_base_currency("default_profile") {
...
fn collect_activity_symbols(
    ...
    if let Some(currency) = activity_currency {
        match context.account_service().get_account(account_id, "default_profile") {
            Ok(account) => {
...
fn handle_portfolio_calculation(
    ...
    spawn(async move {
        ...
        let initially_targeted_active_accounts: Vec<String> =
            match account_service.list_accounts(Some(true), account_ids_input.as_deref(), "default_profile") {
        ...
        if let Err(e) = snapshot_service
            .calculate_total_portfolio_snapshots("default_profile")
            .await
        {
