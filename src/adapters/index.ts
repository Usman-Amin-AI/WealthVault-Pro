...
import type { EventCallback } from "./tauri";
...
export const listenLivePriceUpdate = (handler: EventCallback<unknown>) => {
  return getRunEnv() === RUN_ENV.DESKTOP
    ? tauri.listenLivePriceUpdateTauri(handler)
    : web.listenLivePriceUpdateWeb(handler);
};
