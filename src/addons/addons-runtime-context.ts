import type { AddonContext, SidebarItemHandle } from "@investwise/addon-sdk";
      ...
      const baseAPI = createSDKHostAPIBridge(
        {
          ...
          navigateToRoute: async (route: string) => {
            ...
            const navigate = (
              window as unknown as { __investwise_navigate__?: (r: string) => void }
            ).__investwise_navigate__;
            if (navigate) {
              navigate(route);
            } else {
          ...
          getQueryClient: () => {
            ...
            return (window as unknown as { __investwise_query_client__?: QueryClientLike })
              .__investwise_query_client__;
          },
          invalidateQueries: (queryKey: string | string[]) => {
            ...
            const queryClient = (
              window as unknown as { __investwise_query_client__?: QueryClientLike }
            ).__investwise_query_client__;
            if (queryClient) {
