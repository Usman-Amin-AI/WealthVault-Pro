...
export const startLiveStream = async (symbols: string[], provider: string): Promise<void> => {
  try {
    switch (getRunEnv()) {
      case RUN_ENV.DESKTOP:
        await invokeTauri("start_live_stream", { symbols, provider });
        return;
      case RUN_ENV.WEB:
        await invokeWeb("start_live_stream", { symbols, provider });
        return;
      default:
        throw new Error(`Unsupported environment`);
    }
...
export const getLivePrices = async (symbols: string[]): Promise<Record<string, LivePrice>> => {
  try {
    switch (getRunEnv()) {
      case RUN_ENV.DESKTOP:
        return invokeTauri("get_live_prices", { symbols });
      case RUN_ENV.WEB:
        return invokeWeb("get_live_prices", { symbols });
      default:
        throw new Error(`Unsupported environment`);
    }
  } catch (error) {
    logger.error("Error fetching live prices.");
    throw error;
  }
};
